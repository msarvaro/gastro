...
func (c *AuthController) GoogleLogin(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		log.Printf("Error decoding Google login request: %v", err)
		http.Error(w, "Invalid request body", http.StatusBadRequest)
		return
	}

	response, err := c.userService.GoogleLogin(r.Context(), req)
	if err != nil {
		switch err {
		case user.ErrInvalidCredentials:
			http.Error(w, "Invalid Google credentials or user not found", http.StatusUnauthorized)
		case user.ErrUserInactive:
			http.Error(w, "User is inactive", http.StatusForbidden)
		case user.ErrTokenGeneration:
			http.Error(w, "Internal server error", http.StatusInternalServerError)
	...
	if response.BusinessID > 0 {
		...
		log.Printf("Google login controller: Business ID cookie set to: %d", response.BusinessID)
	}
